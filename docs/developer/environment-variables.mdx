...
- **OSS**: The default mode. Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get Elphie running. Uses local JWT authentication and MinIO for storage.
...
| `MPS_API_URL` | `https://services.elphie.com` | Elphie Managed Platform Services URL |
| `ELPHIE_MPS_SECRET_KEY` | `null` | **Required for non-OSS deployments.** Secret key for authenticating with MPS |
