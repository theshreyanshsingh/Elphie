...
description: "Complete reference for all environment variables used by the Elphie backend"
---
...
Elphie supports two deployment modes, set via `DEPLOYMENT_MODE`:

- **Self-hosted**: The default mode (`DEPLOYMENT_MODE=selfhosted`). Designed for self-hosted deployments using [Docker Compose](/deployment/docker) — the fastest way to get Elphie running. Uses local JWT authentication and MinIO for storage.
- **SaaS**: Intended for customised deployments outside of Docker. Authentication and API key management are handled through Elphie Managed Platform Services (MPS), allowing greater flexibility in how the platform is hosted and integrated.
...
| `DEPLOYMENT_MODE` | `selfhosted` | Deployment mode. Use `selfhosted` for self-hosted Docker installs; `saas` for managed/custom |
...
| Variable | Default | Description |
|---|---|---|
| `SELFHOSTED_JWT_SECRET` | N/A | **Required for self-hosted deployments.** Secret used to sign JWT tokens. Must be set to a strong random value in production. Legacy alias: `OSS_JWT_SECRET` (still read if the new name is unset) |
| `SELFHOSTED_JWT_EXPIRY_HOURS` | `720` | JWT token lifetime in hours (default: 30 days). Legacy alias: `OSS_JWT_EXPIRY_HOURS` |
...
<Warning>
Never use the placeholder `SELFHOSTED_JWT_SECRET` in a production deployment. Generate a strong random secret and store it securely.
</Warning>
...
| `MPS_API_URL` | `https://services.dograh.com` | Managed Platform Services URL used for signup auto-keys and managed LLM/TTS/STT. Override to point at your own MPS (or keep Dograh's public stack for trial keys) |
| `ELPHIE_MPS_SECRET_KEY` | `null` | **Required for non-self-hosted (SaaS) deployments.** Secret key for authenticating with MPS. `DOGRAH_MPS_SECRET_KEY` is accepted as a legacy alias |
| `CORS_ALLOWED_ORIGINS` | `null` | **Required for non-self-hosted (SaaS) deployments.** Comma-separated list of origins allowed to make credentialed cross-origin requests (e.g. `https://app.example.com,https://admin.example.com`). Ignored in self-hosted mode, which serves a permissive same-origin policy without credentials |

---

## Storage

Elphie uses **MinIO by default**, which is bundled with the self-hosted deployment and requires no external setup. Set `ENABLE_AWS_S3=true` to switch to AWS S3 — typically used for cloud or managed deployments where S3 is already part of the infrastructure.
...
<Note>
Presigned URLs point at `S3_ENDPOINT_URL`, so that host must be reachable from the browser. Because browsers fetch transcripts cross-origin, the bucket also needs a CORS rule allowing your app's origin for `GET`/`HEAD` — configure this on the storage server (e.g. via `PutBucketCors`), not in Elphie.
</Note>
...
Controls concurrency for [Campaigns](/core-concepts/campaigns), Elphie's bulk outbound calling feature.
