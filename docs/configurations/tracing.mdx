...
[Langfuse](https://langfuse.com/docs) is an open-source LLM observability platform — it stores and visualizes traces (prompts, responses, tool calls) so you can debug and iterate on LLM-powered applications outside of Elphie's own trace viewer.

We provide seamless integration with Langfuse for tracing if you want to use your own account. This enables you to use the [playground feature of Langfuse](https://langfuse.com/docs/prompt-management/features/playground). This works on both managed and self-hosted Elphie deployments.

**Setup steps:**

1. Sign up at [Langfuse](https://langfuse.com) and create API credentials
2. In the Elphie UI, go to **Platform Settings** (`/settings`) and enter your Langfuse host, public key, and secret key
3. Click **Save**

Once enabled, traces will be available for every completed call in Elphie.
