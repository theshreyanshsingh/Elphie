...
description: "Configure the speech-to-speech, Elphie-managed, or bring-your-own-key models your Elphie agents use."
---
...
To configure models, open **Models** in your Elphie dashboard:

- **Hosted:** `https://elphie.willowave.in/model-configurations`
...
| **Elphie** | Use Elphie-managed LLM, voice, and transcriber models behind one Elphie Service Key. |
...
<Warning>
An LLM is still required when you use Speech to Speech. The realtime model handles the live voice conversation, but Elphie uses the LLM for analysis tasks that happen outside the live audio stream.
</Warning>

## Elphie

Use **Elphie** when you want Elphie to manage the model providers for you. This path uses one Elphie Service Key for Elphie-managed models instead of separate provider keys for LLM, Voice, and Transcriber.

![Elphie model configuration](../images/model-configuration-elphie.png)

Configure:

| Field | What it controls |
|-------|------------------|
| **Voice** | The Elphie-managed voice to use. |
| **Speed** | The voice playback speed. |
| **Language** | The language behavior, including multilingual auto-detect when available. |
| **API Key** | Your Elphie Service Key. Create Service Keys from **Developers**. |
