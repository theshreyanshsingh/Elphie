...
Elphie exposes two public outbound agent run route families. They are **not**
...
to `/public/agent/workflow/{workflow_uuid}`. Elphie treats these as different
identifier types and the request will fail if you mix them up.
</Note>

Once Elphie resolves the target agent, both route families behave the same:
