...
description: Add your Elphie voice agent to any website so visitors can talk to it.
tag: "NEW"
---
...
| **Inline Component**  | A panel rendered inside a `<div id="elphie-inline-container">` that you place in your page.        | You want the agent embedded in a specific section (landing-page hero, support tab, etc.).            |
| **Headless**          | No UI. Only the audio pipeline plus a JavaScript API on `window.ElphieWidget`.                     | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `elphie-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.ElphieWidget`. Code that registers callbacks must wait for the widget to be available.
...
```html
<!-- Paste the elphie embed snippet from the dashboard somewhere on the page -->
<div id="elphie-inline-container"></div>
```

### React

Because React mounts after the widget script may have already loaded, integrate via `initInline` on first mount and `refresh` on remount. Poll for `window.ElphieWidget` to handle the async script load.
...
export function Assistant() {
  useEffect(() => {
    let retries = 0;
    const tryInit = () => {
      const container = document.getElementById('elphie-inline-container');
      if (window.ElphieWidget && container) {
        const { isInitialized } = window.ElphieWidget.getState();
        if (isInitialized) window.ElphieWidget.refresh();
        else window.ElphieWidget.initInline({ container });
      } else if (retries++ < 50) {
  ...
  return <div id="elphie-inline-container" />;
}
```
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.ElphieWidget.start()`                  | Begin a voice call. Must be called from inside a user-gesture handler (e.g. `click`) so the browser grants microphone access.                                |
| `window.ElphieWidget.end()`                    | End the active call.                                                                                                                                         |
| `window.ElphieWidget.onCallStart(cb)`          | Fires when `start()` is invoked (status `connecting`). No payload.                                                                                           |
| `window.ElphieWidget.onCallConnected(cb)`      | Fires when the WebRTC connection is established. Payload: `{ agentId, workflowRunId, token }`.                                                               |
| `window.ElphieWidget.onCallDisconnected(cb)`   | Fires only if the call had connected, when teardown runs. Payload: `{ agentId, workflowRunId, token, durationSeconds }`.                                     |
| `window.ElphieWidget.onCallEnd(cb)`            | Fires whenever the call session is torn down (including failed-to-connect attempts). No payload.                                                             |
| `window.ElphieWidget.onStatusChange(cb)`       | Fires on every status change. Callback receives `(status, text, subtext)`. Status values: `idle`, `connecting`, `connected`, `failed`.                       |
| `window.ElphieWidget.onError(cb)`              | Fires on errors (mic permission denied, server error, etc.). Callback receives an `Error` object.                                                            |

All `on*` setters are single-listener — calling the same one again replaces the previous handler.

<Note>
**About timing.** The widget script loads asynchronously, so `window.ElphieWidget` may not exist at the moment your inline `<script>` first runs. The examples below assume `window.ElphieWidget` is already available when registration runs. To guarantee that:
...
<script>
  ...
  window.ElphieWidget.onError((err) => {
    console.error('Elphie error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    window.ElphieWidget.onStatusChange((s) => setStatus(s));
    window.ElphieWidget.onError((err) => console.error('Elphie error:', err.message));
  }, []);
...
```js
window.ElphieWidget.onCallConnected(({ agentId, workflowRunId }) => {
  analytics.track('voice_call_started', { agentId, workflowRunId });
});

window.ElphieWidget.onCallDisconnected(({ workflowRunId, durationSeconds }) => {
