...
When the LLM decides a transfer is needed, it calls the Call Transfer tool. Elphie resolves the destination, validates the transfer setup, and then starts the provider transfer.

For a successful transfer:

1. The agent calls the Call Transfer tool.
2. Elphie resolves the destination from either static configuration or a dynamic HTTP resolver.
3. Elphie optionally plays a pre-transfer message.
4. Elphie starts dialing the destination through the telephony provider.
5. The caller hears hold music while the destination leg is connecting.
6. When the destination answers, Elphie connects the caller and ends the AI agent's involvement.

The transfer is a blind transfer. Elphie does not currently pass conversation context to the receiving human as part of the transfer.
...
Elphie sends a `POST` request to your resolver endpoint before starting the transfer. Your endpoint returns the destination and may return a custom message to play before dialing.
...
- Use business logic from your backend instead of hardcoding numbers in Elphie

## Dynamic resolver request

The resolver request body is a flat JSON object.

Elphie builds it from:

- **LLM parameters**: values the agent extracts from the conversation when it calls the transfer tool
- **Preset parameters**: values Elphie injects from fixed values or templates such as `{{initial_context.account_id}}` or `{{gathered_context.billing_issue_type}}`
...
<Note>
Elphie does not send the full conversation transcript to the resolver. Add the specific values you need as LLM parameters or preset parameters.
</Note>
...
<Warning>
Follow this response shape exactly. Elphie reads specific keys from the resolver response. For dynamic transfer resolution, an invalid response does not fall back silently: the transfer fails before dialing.
</Warning>
...
- **Resolver URL**: HTTPS or HTTP endpoint Elphie calls with `POST`
- **Resolver Timeout**: How long Elphie waits for the resolver response
- **Resolver Wait Message**: Optional message spoken while Elphie waits, such as "One moment while I find the right team."
- **LLM Parameters**: Values the agent should extract from the conversation, such as `billing_issue_type` or `requested_department`
- **Preset Parameters**: Values injected by Elphie from context or fixed values
...
1. Elphie resolves the static or template destination.
2. Elphie plays the configured pre-transfer message, if any.
3. Elphie starts the provider transfer.

For dynamic transfers:

1. Elphie may play the resolver wait message while calling your resolver.
2. If the resolver returns `custom_message`, Elphie plays that message.
3. If the resolver does not return `custom_message`, Elphie uses the configured pre-transfer message.
4. Elphie starts the provider transfer.

Resolver `custom_message` overrides the configured pre-transfer message for that transfer attempt.

## Transfer timeout

Transfer timeout is the maximum time Elphie waits for the destination to answer after the provider transfer starts.

This timeout applies to both static and dynamic transfers.

The resolver timeout is separate. It controls only how long Elphie waits for your HTTP resolver before dialing starts.
...
- a preset parameter, if Elphie should inject it from context
