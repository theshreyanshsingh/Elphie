...
| Field | Description |
| --- | --- |
| **Endpoint URL** | The URL Elphie will send the POST request to. |
...
Elphie sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/elphie/pre-call", async (req, res) => {
