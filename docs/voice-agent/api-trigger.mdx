...
If you are using the hosted version, replace `your-elphie-instance` with `api.elphie.com`.
...
The id is shown on each row in **Telephony configurations** (`https://elphie.willowave.in/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
