...
The API Trigger node lets you initiate outbound calls to your voice agent programmatically. When you add an API Trigger node to your workflow, Elphie generates a unique endpoint URL that external systems can call to start a conversation.
...
When you add an API Trigger node to your workflow, Elphie assigns it a unique UUID. The trigger node exposes two URLs that share this UUID:
...
```
POST https://your-elphie-instance/api/v1/public/agent/{uuid}        # Production
POST https://your-elphie-instance/api/v1/public/agent/test/{uuid}   # Test
```

If you are using the hosted version, replace `your-elphie-instance` with `elphie.nearzero.dev`.
...
<CodeGroup>
```bash Production URL
curl -X POST https://your-elphie-instance/api/v1/public/agent/{uuid} \
...
```bash Test URL
curl -X POST https://your-elphie-instance/api/v1/public/agent/test/{uuid} \
  -H "Content-Type: application/json" \
  -H "X-API-Key: dg_your_api_key" \
  -d '{
    "phone_number": "+14155550100",
...
The id is shown on each row in **Telephony configurations** (`https://elphie.nearzero.dev/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
