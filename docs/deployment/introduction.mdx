---
title: "Deployments"
description: "Options for self-hosting Elphie — from a local Docker install to a production remote server with a custom domain."
---

Elphie is self-hosted with Docker. Everything ships as two images (`elphie-api` and `elphie-ui`) plus Postgres, Redis, and MinIO, orchestrated with Docker Compose. Pick the deployment path that matches what you're trying to do:

| Path | Use it when | Guide |
| --- | --- | --- |
| **Local Docker** | You want to try Elphie on your own machine. No public URL, no telephony. | [Docker → Option 1](/deployment/docker#option-1-local-docker-deployment) |
...
- [**Update**](/deployment/update) — move an existing deployment to a newer Elphie version.
...
- [Contribution: Setup](/contribution/setup) — for developing on Elphie itself rather than deploying it
