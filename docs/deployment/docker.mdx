...
From a local checkout or distribution of Elphie (with `docker-compose.yaml` at the repo root), run the starter script and confirm the prompt to start Elphie AI:
...
This setup:
- Uses the local `docker-compose.yaml`
- Creates `SELFHOSTED_JWT_SECRET`, `REDIS_PASSWORD`, and MinIO credentials in `.env` if they do not already exist
...
For these cases, use the alternate local setup script from your checkout, which configures a coturn TURN server alongside the rest of the stack:
...
From a local checkout or distribution, run the automated setup script that will configure everything for you:
...
- Use the local `docker-compose.yaml` (prebuilt mode), or prepare a full source tree for image builds (build mode)
- Set up the validated remote deployment helper bundle
...
After the setup script completes, start Elphie. For a **public-IP install the trusted-certificate flow already started the stack** and printed your `https://…sslip.io` URL — you can skip to [Access Your Application](#access-your-application). For a self-signed install, start it yourself (the script prints the exact command at the end — it differs slightly between modes):
...
If you maintain a customized tree or want to ship local code changes without waiting for an official image release, pick **build** when the setup script prompts for deployment mode.

In this mode the script:

1. Uses your existing Elphie checkout when you run it from inside one (recommended).
2. Otherwise prompts for a source location so you can point at your own distribution or branch.
...
Whenever the codebase changes — a local edit, an updated distribution, or a submodule bump — rebuild the `api` and `ui` images and recreate the running containers so they pick up the new code.
...
**Step-by-step** — use this when you've refreshed the source tree, or want to be explicit:

```bash
cd elphie

# 1. Refresh local source / submodules as needed.
...
<Note>
Build mode is for **self-hosters who want to deploy customized images** — not for active development on the code itself. For day-to-day contributor work (fast reload, IDE/LSP integration), use the [contributor setup](/contribution/setup) instead.
</Note>
