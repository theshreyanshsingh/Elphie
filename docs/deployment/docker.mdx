---
title: "Docker"
description: "Deploy Elphie AI using Docker for local development and remote server environments"
---

Elphie AI can be deployed using Docker in two main configurations. Choose the option that best fits your needs:

- **Option 1**: For local development and testing on your own machine
- **Option 2**: For remote server deployment with HTTPS (using IP address). If you also have a custom domain, you can first deploy Elphie stack on your server using steps in this document and then proceed to the [Custom Domain](deployment/custom-domain) section.

<Tip>
**Using Claude Code or Codex?** Install the official Elphie setup skill and let your agent drive either deployment below — it orients to your OS, picks local vs remote, runs Elphie's own setup scripts, and verifies the result with a built-in health check.
...
Start a new session, then ask it to *"set up Elphie"* (or run `/elphie-setup` in Claude Code). More at [elphie-hq/elphie-plugins](https://github.com/elphie-hq/elphie-plugins).
</Tip>

## Option 1: Local Docker Deployment

Watch the video tutorial below for a step-by-step walkthrough of setting up Elphie AI locally with Docker.

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.tella.tv/video/elphie-local-setup-with-docker-5wsf/embed?b=1&title=1&a=1&loop=0&t=0&muted=0&wt=1"
  title="Elphie Local Setup with Docker"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowFullScreen
></iframe>

For local development and testing, you can run Elphie AI directly on your machine using Docker with a small startup script.

### Quick Start

Download the compose file and starter script, then confirm the prompt to start Elphie AI:
...
<CodeGroup>
```bash macOS/Linux
curl -o setup_local.sh https://raw.githubusercontent.com/elphie-hq/elphie/main/scripts/setup_local.sh && chmod +x setup_local.sh && ./setup_local.sh
```
```powershell Windows
Invoke-WebRequest -OutFile setup_local.ps1 https://raw.githubusercontent.com/elphie-hq/elphie/main/scripts/setup_local.ps1
.\setup_local.ps1
```
</CodeGroup>
...
It creates `docker-compose.yaml`, a `.env` file with JWT and TURN credentials, and the small helper bundle that `elphie-init` uses to render coturn config at startup. Start the stack with the `local-turn` profile so coturn comes up alongside the other services:
...
```bash
curl -o setup_remote.sh https://raw.githubusercontent.com/elphie-hq/elphie/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh
```
...
- Validate the runtime config that `elphie-init` will render from `.env`
...
After the setup script completes, start Elphie. The script prints the exact command to run at the end — it differs slightly between modes:
...
The setup script creates the following files in the `elphie/` directory:
...
| `scripts/run_elphie_init.sh` | One-shot init renderer/validator used by Docker Compose |
...
1. Clones the repo into `elphie/` (default: `elphie-hq/elphie@main`; the script will ask for an `owner/name` and `branch` so a fork can point at its own).
2. If you run the script from inside an existing elphie checkout, offers to use that checkout instead of cloning.
