---
title: "Custom Domain"
description: "Deploy Elphie AI with custom domain names and SSL certificates"
---

Deploy Elphie AI with your own custom domain name for a professional production setup. By now, you should be able to create and test a voice agent by following the previous guide to setup the platform on a remote server using [Docker](docker#option-2%3A-remote-server-deployment)

## What is Custom Domain Deployment?

Custom domain deployment allows you to run Elphie AI with a personalized domain name (like `voice.yourcompany.com`) instead of using IP addresses. This setup includes:
...
- Elphie AI already running on your server via the [remote deployment](docker#option-2%3A-remote-server-deployment) guide
...
<Note>
You must be at the same place where you had run `setup_remote.sh` from. The directory should contain `elphie/` with the artifacts that got created when `setup_remote.sh` was run.
</Note>
<Note>
You must not move the `elphie/` directory to a different location after running `setup_custom_domain.sh`, since we set up auto certificate renewal script as certbot renewal hook pointing to the `elphie/` directory.
</Note>

```bash
curl -o setup_custom_domain.sh https://raw.githubusercontent.com/elphie-hq/elphie/main/scripts/setup_custom_domain.sh && chmod +x setup_custom_domain.sh && sudo ./setup_custom_domain.sh
```
...
- Restart Elphie services through the validated startup wrapper
...
### Stop Elphie Services

Before generating certificates, stop the running Elphie services to free up port 80:
...
### Copy Certificates to Elphie Directory

Copy the generated certificates to the elphie certs directory:
...
### Start Elphie Services

Start Elphie through the validated startup wrapper so `elphie-init` regenerates nginx and coturn runtime config before Docker starts:
...
```bash
sudo nano /etc/letsencrypt/renewal-hooks/deploy/elphie-reload.sh
```
...
```bash
#!/bin/bash
# Copy renewed certificates to elphie certs directory
cp /etc/letsencrypt/live/voice.yourcompany.com/fullchain.pem /home/ubuntu/elphie/certs/local.crt
cp /etc/letsencrypt/live/voice.yourcompany.com/privkey.pem /home/ubuntu/elphie/certs/local.key
chmod 644 /home/ubuntu/elphie/certs/local.crt /home/ubuntu/elphie/certs/local.key

# Restart nginx to load new certificates
cd /home/ubuntu/elphie
...
```bash
sudo chmod +x /etc/letsencrypt/renewal-hooks/deploy/elphie-reload.sh
```
...
1. Verify the certificates were copied correctly: `ls -la elphie/certs/`
2. Run `./remote_up.sh --preflight-only` in `elphie/` to verify the `elphie-init` runtime render matches `.env`
