...
```bash
curl -o setup_custom_domain.sh https://raw.githubusercontent.com/theshreyanshsingh/Elphie/main/scripts/setup_custom_domain.sh && chmod +x setup_custom_domain.sh && sudo ./setup_custom_domain.sh
```
...
Update `.env` so the canonical remote settings use your domain — `elphie-init` reads these to render nginx's `server_name`. Replace `voice.yourcompany.com` with your actual domain throughout:
