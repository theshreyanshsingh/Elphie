...
description: "Configure how self-hosted Elphie authenticates users — the default local provider, or Stack Auth for social login"
---

Self-hosted Elphie ships with a built-in **local** authentication provider (email + password, backed by a signed JWT). This is the default and needs no external service.

To offer social logins (Google, GitHub, and others), you can delegate sign-in to **[Stack Auth](https://stack-auth.com)**. Enabling it is a **runtime** configuration change — set a few environment variables and restart. The prebuilt `elphieai/elphie-api` and `elphieai/elphie-ui` images work as-is; you do **not** need to rebuild or build from source.
