...
**Telephony**
The phone infrastructure. Elphie connects to your telephony provider (Twilio, Vonage, etc.) to place and receive calls. The audio streams between the caller and Elphie in real time.

**Transcriber (STT)**
Converts the caller's speech to text in real time. Elphie sends the audio stream to your configured speech-to-text provider and uses the transcript to drive both the LLM and the final run record.
...
1. Elphie instructs your telephony provider to dial the number
...
6. When an edge condition is met, Elphie transitions to the next node
