...
Elphie has a simple data model for passing information through a call. Understanding it is key to building agents that feel personalised and to extracting useful results after a call.
...
When the call starts, Elphie substitutes the values before sending the prompt to the LLM — so the agent speaks naturally as if it already knows the contact.
...
For telephony calls (inbound and outbound), Elphie automatically adds these variables to `initial_context`:
...
For **inbound** calls, `caller_number` is the customer's number and `called_number` is your Elphie number. For **outbound** calls, it's the reverse — `caller_number` is your Elphie number and `called_number` is the customer's number.
