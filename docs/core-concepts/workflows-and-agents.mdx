...
description: "How conversation flows are defined in Elphie"
---

In Elphie, what you see as an **agent** in the dashboard is called a **workflow** in the API. They are the same thing — a workflow is the underlying definition, agent is the product name for it.
...
Every time you update a workflow's `workflow_definition`, Elphie saves a new version while keeping the history. The current version is always what runs. Old versions are retained for auditing.
...
- **From a template** — describe the use case in natural language and Elphie generates the initial graph using an LLM. Best for getting started quickly.
