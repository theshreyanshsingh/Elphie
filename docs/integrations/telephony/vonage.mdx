...
description: "Configure Vonage (Nexmo) for voice communication in Elphie AI"
---

## Overview

Vonage (formerly Nexmo) is a cloud communications platform that provides global voice, messaging, and video capabilities. Elphie AI's Vonage integration enables high-quality voice interactions with your agents using Vonage's robust infrastructure.
...
2. Copy your **Signature Secret** from the same Vonage account. Elphie uses it
...
Vonage routes inbound Voice API calls through a Voice application. The application owns the Answer URL and Event URL, and the phone number must be linked to that application. Elphie routes the call to the right agent based on the called number's inbound workflow assignment inside Elphie.

When you save an inbound workflow on a phone number, Elphie updates the configured Vonage application's Voice webhooks and enables signed callbacks, provided the API Key, API Secret, and Application ID are correct and a Signature Secret is configured.

<Warning>
  Linking the phone number to the Voice application is required. If the number
  is not linked, Vonage will not call Elphie's Answer URL, and you may hear a
  busy or disconnected tone without seeing any Elphie application logs.
</Warning>
...
4. Set the number's Voice application to the same Vonage Application whose ID you configured in Elphie
...
### Step 2: Assign an Inbound Workflow to the Phone Number in Elphie
...
2. Under **Capabilities** → **Voice**, confirm:
   ...
   <Note>
     Elphie pushes these settings automatically when you save the inbound
     workflow in Step 2. If the fields are empty, show a different URL, or
     Elphie surfaced a sync warning on save, check the API Key, API Secret,
     Application ID, and Signature Secret in Elphie, then save the inbound
     workflow again. On self-hosted Elphie, the backend domain must be publicly
     reachable by Vonage.
   </Note>
...
- Use the [Vonage logs](https://dashboard.nexmo.com/logs) or Voice Inspector to confirm Vonage is sending the Answer webhook to Elphie
...
3. Check call logs in both Elphie AI dashboard and Vonage Dashboard
...
<AccordionGroup>
  ...
  <Accordion title="Signed webhook validation failed">
    - Verify the Signature Secret in Elphie matches the Vonage account's signature secret
    ...
    - Confirm the application belongs to the same API Key saved in Elphie
  </Accordion>
  ...
  <Accordion title="Inbound calls not reaching voice agent">
    ...
    - Confirm the called number exists in your Elphie telephony configuration and has an **Inbound workflow** assigned
  </Accordion>

  <Accordion title="Inbound call gives busy tone and Elphie shows no logs">
    - Confirm the Vonage number is linked to the Voice application configured in Elphie
    - Confirm the Voice application has the Elphie Answer URL set with method `POST`
    - Confirm your backend domain is public; Vonage cannot call `localhost`
    - Check Vonage logs for Answer URL delivery errors before debugging Elphie
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    - Verify API Key matches the one stored in your Elphie telephony configuration (used to identify the org from the inbound webhook)
...
- **Signed callbacks**: Keep Vonage signed callbacks enabled and keep the Signature Secret in Elphie up to date
