...
description: "Connect Elphie AI to your Asterisk PBX using the Asterisk REST Interface (ARI)"
---

## Overview

Asterisk ARI (Asterisk REST Interface) allows you to connect Elphie AI voice agents to your existing Asterisk PBX. ARI provides a WebSocket-based event model for controlling calls via Stasis applications, giving Elphie full control over call flow and audio streaming.

This guide focuses on the Elphie-specific configuration. For general Asterisk installation and administration, refer to the [official Asterisk documentation](https://docs.asterisk.org/).
...
- Network connectivity between your Elphie instance and Asterisk
- Elphie AI instance running and accessible

<Note>
If you compiled Asterisk from source, ensure both `chan_websocket` and `res_websocket_client` are included during the build. These modules are required for external media streaming between Asterisk and Elphie. Refer to the [Asterisk build system documentation](https://docs.asterisk.org/) for details on enabling modules.
</Note>

## Asterisk Configuration

The following Asterisk configuration files need to be set up to work with Elphie. These are minimal examples focused on the Elphie integration -- refer to the [Asterisk documentation](https://docs.asterisk.org/) for full configuration details.

### Enable ARI (`ari.conf`)

Create an ARI user that Elphie will use to authenticate:
...
<Note>
The username (section name, e.g., `elphie`) and password here must match the **Stasis App Name** and **App Password** you configure in Elphie.
</Note>
...
Route incoming calls to your Stasis application so Elphie can handle them:

```ini
[from-external]
exten => _X.,1,NoOp(Incoming call to ${EXTEN})
 same => n,Stasis(elphie)
 same => n,Hangup()
```

Replace `elphie` with the app name you configured in `ari.conf` and in Elphie.

### Configure External Media Streaming (`websocket_client.conf`)

Elphie uses Asterisk's external media streaming to send and receive audio over WebSocket. Configure a WebSocket client connection that points to your Elphie instance:

<Tabs>
  <Tab title="Elphie Cloud">
    ```ini
    [elphie]
    type = websocket_client
    uri = wss://api.elphie.com/api/v1/telephony/ws/ari
    protocols = media
    tls_enabled = yes
    ...
    <Note>
    `tls_enabled = yes` is required even though the URI scheme is `wss://` — without it Asterisk will not negotiate TLS and the connection will fail. The ARI credentials (**Stasis App Name** and **App Password**) must match what you configure in the Elphie dashboard under Telephony Settings.
    </Note>
  </Tab>
  <Tab title="Self-hosted">
    ...
    [elphie]
    type = websocket_client
    uri = ws://your-elphie-host:port/api/v1/telephony/ws/ari
    protocols = media
    ```

    <Note>
    Self-hosted deployments on an internal network may use an unencrypted WebSocket (`ws://`). If your Elphie instance is exposed over HTTPS, use `wss://` and the corresponding hostname instead.
    </Note>
  </Tab>
</Tabs>

<Note>
The section name (e.g., `elphie`) is the **WebSocket Client Name** you'll enter in the Elphie telephony configuration. This name tells Asterisk which WebSocket connection to use for external media streaming during calls.
</Note>

<Note>
Configure the `uri` as a base URL only, without a query string. During each call, Elphie asks Asterisk to create an `externalMedia` channel and Asterisk appends `workflow_id`, `organization_id`, and `workflow_run_id` through the `v()` transport data for that call. Opening `/api/v1/telephony/ws/ari` directly in a browser or with `wscat` can return HTTP 403 because those routing parameters are missing; that is expected and does not indicate a `websocket_client.conf` misconfiguration.
</Note>

<Note>
Elphie's external media channel uses **G.711 μ-law (`ulaw`)**. Make sure any PJSIP endpoint or SIP trunk that places or receives calls through Elphie allows `ulaw` (e.g. `allow=ulaw` in the endpoint config).
</Note>
...
| **Stasis App Name** | The ARI username configured in `ari.conf` | `elphie` |
| **App Password** | The ARI password configured in `ari.conf` | `your_secure_password` |
| **WebSocket Client Name** | The connection name from `websocket_client.conf` | `elphie` |
...
Unlike other telephony providers that use HTTP webhooks for inbound calls, ARI delivers inbound calls as **StasisStart events on the ARI WebSocket**. Elphie automatically detects these events and activates the workflow assigned to the called extension.

### How It Works

1. An external call arrives at Asterisk and the dialplan routes it to `Stasis(elphie)`
...
3. Elphie looks up the called extension in your telephony configuration's phone numbers, finds the assigned workflow, validates quota, and creates a workflow run
...
…or use a pattern that catches every extension you'll register in Elphie:
...
**Step 2: Add the extension as a phone number in Elphie**
...
4. Save
   ...
   <Note>
   Adding the extension in Elphie doesn't change Asterisk's dialplan — that's
   what Step 1 is for. The Elphie entry tells the StasisStart handler which
...
<AccordionGroup>
  <Accordion title="Cannot connect to ARI endpoint">
    - Verify the ARI endpoint URL is correct and reachable from your Elphie instance
  ...
  <Accordion title="No audio during calls">
    ...
    - Check that `websocket_client.conf` is correctly configured with the right Elphie URI
    - Ensure the WebSocket Client Name in Elphie matches the section name in `websocket_client.conf`
    - Verify network connectivity and firewall rules allow WebSocket traffic between Asterisk and Elphie
  </Accordion>

  <Accordion title="Calls not reaching Elphie">
  ...
  <Accordion title="Inbound calls are immediately hung up">
    ...
    - Review Elphie logs for warnings like "no matching phone number registered
    for config" or "has no inbound_workflow_id assigned"
  </Accordion>

  <Accordion title="WebSocket client connection issues">
    - Check the URI in `websocket_client.conf` points to the correct Elphie host and port
    - Verify the Elphie instance is running and accepting WebSocket connections
...
- Keep your Asterisk instance on the same network or a low-latency connection to Elphie for optimal audio quality
- Use strong passwords for ARI authentication
- Restrict ARI access to known IP addresses using firewall rules
- Monitor Asterisk logs alongside Elphie logs when debugging call issues
