...
description: "Configure Telnyx for voice communication in Elphie AI"
---

## Overview

Telnyx is a cloud communications platform that provides programmable voice via its Call Control API. Elphie AI's Telnyx integration uses Call Control plus WebSocket media streaming to power your voice agents.
...
- A **Call Control Application** (optional — leave the field blank in Elphie and we'll auto-create one for you on save, with the inbound webhook URL pre-set)
...
3. (Optional) Navigate to **Call Control** → **Applications** and create (or open) the application you'll use with Elphie, then copy its **Connection ID** (Call Control App ID). Skip this if you want Elphie to auto-create the Call Control Application for you on save.
4. Navigate to **Numbers** → **My Numbers** and assign your phone numbers to the Call Control Application you'll use with Elphie (if you're letting Elphie auto-create the application, do this after saving the configuration in Step 2)
...
3. Enter your credentials:
   - API Key
   - Call Control App ID (Connection ID) — *optional*. Leave blank and Elphie will auto-create a Call Control Application on save (with the inbound webhook URL already configured) and store its Connection ID on this configuration.
...
5. Open the configuration you just created and add at least one **phone number** (with country code in E.164 format, e.g. `+1234567890`). The default caller ID is used for outbound calls.
   ...
   <Note>
     If Elphie auto-created the Call Control Application for you, you still
     ...
     is named `elphie-<random>` — its Connection ID is shown on the saved
     configuration.
   </Note>
...
Telnyx delivers inbound webhooks at the **Call Control Application** level — the webhook URL is configured once on the application, and applies to every number assigned to it. **When you save an inbound workflow on a phone number, Elphie automatically pushes the webhook URL to your Call Control Application's `webhook_event_url`** (provided the credentials are correct). If Elphie auto-created the application during configuration save, the webhook URL is already set and this step is a no-op.
...
2. Open the application whose Connection ID you configured in Elphie
3. In **Webhook Settings**, confirm:
   - **Webhook URL** is set to: `https://elphie.nearzero.dev/api/v1/telephony/inbound/run`
   - **HTTP Method** is `POST`
4. Make sure the phone numbers you want to use for inbound are assigned to this application
   ...
   <Note>
     ...
     Control Application — Elphie matches the inbound call to the right
     agent using the called number's inbound workflow assignment. If the
     field is empty, shows a different URL, or Elphie surfaced a sync
     warning on save, the auto-push failed — most often because the API
     Key or Connection ID in Elphie is incorrect. Paste the URL into the
     field yourself, set the method to `POST`, and save. On self-hosted
     Elphie, replace `elphie.nearzero.dev` with your backend domain.
   </Note>
...
3. Check call logs in both Elphie AI dashboard and Telnyx Portal
...
<AccordionGroup>
  ...
  <Accordion title="Inbound calls aren't answered">
    ...
    `https://elphie.nearzero.dev/api/v1/telephony/inbound/run` - Ensure the webhook
    ...
    Elphie AI instance is running and responding
  </Accordion>
  ...
  <Accordion title="Voice agent doesn't respond to inbound calls">
    ...
    your Elphie telephony configuration - Verify WebSocket connection
...
- Use a dedicated Call Control Application for Elphie so the shared webhook URL doesn't conflict with other systems
