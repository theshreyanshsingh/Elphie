...
description: "How Elphie AI handles telephony webhooks and audio streaming"
---

## Overview

Elphie AI uses webhooks to communicate with telephony providers for call events and audio streaming. For outbound calls, Elphie builds these URLs and hands them to the provider when it dials — you never configure them by hand. For inbound calls, you point your provider at a single dispatcher URL; see [Inbound Calls](/integrations/telephony/inbound).
...
When an outbound call connects, the provider requests instructions. The path is provider-specific, and Elphie appends the routing parameters as a query string:
...
Telnyx and Asterisk ARI have no answer webhook. Telnyx is call-control style — Elphie POSTs the stream and event URLs to Telnyx's API instead of returning markup. ARI streams over a WebSocket only.
...
Providers report their own vocabulary; Elphie normalizes it into a common set of states:
...
A status Elphie does not recognize is passed through unchanged rather than dropped.
...
The `organization_id` segment is the tenant that owns the workflow. Elphie scopes every workflow and workflow-run lookup by it, so a run belonging to one organization can never be served under another's id.
