...
description: "Trigger a Elphie voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the Elphie UI.

## Prerequisites

- A Elphie [API key](/configurations/api-keys) exported as `ELPHIE_API_KEY`
...
with ElphieClient(api_key="YOUR_API_KEY") as client:
...
const client = new ElphieClient({ apiKey: "YOUR_API_KEY" });
