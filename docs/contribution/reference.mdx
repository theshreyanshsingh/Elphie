...
1. Run the contributor bootstrap. It configures git remotes, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
Plain source edits, `ui/package.json`, and the `.env.example` templates do **not** require a rebuild. After refreshing your local source, a quick check:
...
- `origin`: your contribution remote, where you push
- `upstream`: the canonical Elphie remote, where new commits land

If your remotes are not set up yet, run it once (inside the devcontainer is fine):
...
<Note>
Always push feature branches to **`origin`** (your contribution remote), then share the change with maintainers for review. Never push directly to `upstream`.
</Note>
