...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `theshreyanshsingh/Elphie`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates:
...
The checked-in `.devcontainer/` follows the Dev Containers specification, so it should also work in Cursor, JetBrains IDEs, and other compatible editors. Only the VS Code path is actively tested — if something breaks elsewhere, please open a [GitHub issue](https://github.com/theshreyanshsingh/Elphie/issues).
...
- `upstream`: `theshreyanshsingh/Elphie`, where new commits land

If you cloned `theshreyanshsingh/Elphie` directly instead of your fork, run it once (inside the devcontainer is fine):
...
upstream  https://github.com/theshreyanshsingh/Elphie.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `theshreyanshsingh/Elphie:main`. Never push directly to `upstream`.
</Note>
