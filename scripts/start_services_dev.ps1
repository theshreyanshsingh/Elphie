#!/usr/bin/env pwsh
# Start Elphie services in development mode (Windows)
...
$EnvFile    = if ($env:ELPHIE_ENV_FILE) { $env:ELPHIE_ENV_FILE } else { Join-Path $BaseDir 'api/.env' }
...
Write-Host "Starting Elphie Services (DEV MODE) in BASE_DIR: $BaseDir"
