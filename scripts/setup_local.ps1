...
function Download-InitSupportBundle([string]$ProjectDir, [string]$Ref) {
    ...
    Download-BundleFileForRef (Join-Path $ProjectDir 'scripts/run_elphie_init.sh') 'scripts/run_elphie_init.sh' $Ref
...
if ($env:ELPHIE_SKIP_DOWNLOAD -ne '1') {
    if ($UseCoturn) {
...
if ($UseCoturn) {
    Assert-PathExists 'scripts/run_elphie_init.sh' 'scripts/run_elphie_init.sh not found. Re-run setup_local.ps1 without ELPHIE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'scripts/lib/setup_common.sh' 'scripts/lib/setup_common.sh not found. Re-run setup_local.ps1 without ELPHIE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'deploy/templates/turnserver.remote.conf.template' 'deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.ps1 without ELPHIE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
}
