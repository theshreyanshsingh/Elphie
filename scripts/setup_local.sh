...
# Download compose file (skip when ELPHIE_SKIP_DOWNLOAD=1 — e.g. local repo testing).
TOTAL_STEPS=2
...
if [[ "${ELPHIE_SKIP_DOWNLOAD:-}" != "1" ]]; then
    ...
    curl -sS -o docker-compose.yaml https://raw.githubusercontent.com/elphie-hq/elphie/main/docker-compose.yaml
    if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
        elphie_download_init_support_bundle "$(pwd)" "main"
    fi
...
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    [[ -f scripts/run_elphie_init.sh ]] || elphie_fail "scripts/run_elphie_init.sh not found. Re-run setup_local.sh without ELPHIE_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f scripts/lib/setup_common.sh ]] || elphie_fail "scripts/lib/setup_common.sh not found. Re-run setup_local.sh without ELPHIE_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f deploy/templates/turnserver.remote.conf.template ]] || elphie_fail "deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.sh without ELPHIE_SKIP_DOWNLOAD=1, or use a full repo checkout."
fi
...
echo ""
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    echo -e "${YELLOW}To start Elphie with TURN, run:${NC}"
    echo ""
