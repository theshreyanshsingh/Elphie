...
generate_secret() {
    ...
    elphie_fail "Could not generate a secret. Install python3 or openssl, or set missing secrets manually in .env."
}
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║                  Elphie Remote Update                        ║"
echo "║  Refresh deployment files and validate runtime config        ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

[[ -f docker-compose.yaml ]] || elphie_fail "docker-compose.yaml not found in $(pwd)"
[[ -f .env ]] || elphie_fail ".env not found in $(pwd)"
...
if [[ -f docker-compose.override.yaml ]]; then
    ...
    echo ""
    echo -e "${YELLOW}See https://elphie.nearzero.dev/deployment/update#updating-a-source-build${NC}"
    exit 1
fi
...
elphie_load_env_file .env

[[ -n "${TURN_SECRET:-}" ]] || elphie_fail "TURN_SECRET not found in .env"
...
if [[ -z "$TARGET_VERSION" ]]; then
    elphie_info "Fetching latest release tag from GitHub..."
    LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)

    if [[ -z "$LATEST_TAG" ]]; then
        elphie_warn "Could not auto-discover latest tag - defaulting to 'main'."
        LATEST_TAG="main"
    fi
    ...
    if [[ -t 0 ]]; then
        ...
        echo -e "${YELLOW}full git tag (elphie-v1.28.0), or 'main' for the latest deployment files.${NC}"
...
if [[ "$TARGET_VERSION" == "latest" ]]; then
    TARGET_VERSION=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)
    [[ -n "$TARGET_VERSION" ]] || elphie_fail "could not resolve 'latest' to a release tag"
fi
...
case "$TARGET_VERSION" in
    ...
    elphie-*)
        ;;
    v*)
        TRY_TAGS+=("elphie-$TARGET_VERSION")
        ;;
    *)
        TRY_TAGS+=("elphie-v$TARGET_VERSION" "v$TARGET_VERSION" "elphie-$TARGET_VERSION")
        ;;
esac

elphie_info "Validating target version: $TARGET_VERSION..."
RESOLVED_TAG=""
...
[[ -n "$RESOLVED_TAG" ]] || elphie_fail "could not find a git tag matching '$TARGET_VERSION'"

if [[ "$RESOLVED_TAG" != "$TARGET_VERSION" ]]; then
    elphie_success "✓ Resolved '$TARGET_VERSION' to git tag '$RESOLVED_TAG'"
fi
...
case "$TARGET_VERSION" in
    elphie-v*) IMAGE_TAG="${TARGET_VERSION#elphie-v}" ;;
...
if [[ -n "$IMAGE_TAG" ]]; then
    if curl -fsI "https://hub.docker.com/v2/repositories/elphieai/elphie-api/tags/$IMAGE_TAG/" >/dev/null 2>&1; then
        elphie_success "✓ Image tag :$IMAGE_TAG found on Docker Hub"
    else
        elphie_warn "Warning: image tag :$IMAGE_TAG not found on Docker Hub - leaving images at :latest"
        IMAGE_TAG=""
    fi
fi
...
echo -e "  Server IP:        ${BLUE}$(elphie_infer_server_ip "$(pwd)" || echo "unknown")${NC}"
...
if [[ -t 0 && "${ELPHIE_UPDATE_YES:-}" != "1" ]]; then
...
elphie_download_remote_support_bundle "$(pwd)" "$TARGET_VERSION"
rm -f nginx.conf turnserver.conf

if [[ -n "$IMAGE_TAG" ]]; then
    sed -i.tmp -E "s#(elphie-(api|ui)):latest#\1:$IMAGE_TAG#g" docker-compose.yaml
    rm -f docker-compose.yaml.tmp
    elphie_success "✓ docker-compose.yaml updated; images pinned to :$IMAGE_TAG"
else
    elphie_success "✓ docker-compose.yaml updated (image tags left at :latest)"
fi

echo -e "${BLUE}[3/3] Synchronizing environment and validating init-based remote config...${NC}"
elphie_set_env_key .env FASTAPI_WORKERS "$FASTAPI_WORKERS"
if [[ -z "${REDIS_PASSWORD:-}" ]]; then
    elphie_set_env_key .env REDIS_PASSWORD "$(generate_secret)"
    elphie_success "✓ REDIS_PASSWORD created in .env"
fi
if [[ -z "${MINIO_ROOT_USER:-}" ]]; then
    if [[ -n "${MINIO_ACCESS_KEY:-}" ]]; then
        elphie_set_env_key .env MINIO_ROOT_USER "$MINIO_ACCESS_KEY"
        elphie_success "✓ MINIO_ROOT_USER created in .env from existing MINIO_ACCESS_KEY"
    else
        elphie_set_env_key .env MINIO_ROOT_USER "$(generate_minio_root_user)"
        elphie_success "✓ MINIO_ROOT_USER created in .env"
    fi
fi
if [[ -z "${MINIO_ROOT_PASSWORD:-}" ]]; then
    if [[ -n "${MINIO_SECRET_KEY:-}" ]]; then
        elphie_set_env_key .env MINIO_ROOT_PASSWORD "$MINIO_SECRET_KEY"
        elphie_success "✓ MINIO_ROOT_PASSWORD created in .env from existing MINIO_SECRET_KEY"
    else
        elphie_set_env_key .env MINIO_ROOT_PASSWORD "$(generate_secret)"
        elphie_success "✓ MINIO_ROOT_PASSWORD created in .env"
    fi
fi
elphie_prepare_remote_install "$(pwd)"
docker compose config -q
elphie_success "✓ Remote init configuration validated"
...
echo ""
echo -e "  ${BLUE}for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_elphie_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do${NC}"
