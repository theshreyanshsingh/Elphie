// Unit tests for @elphie/sdk. Uses Node's built-in `node:test` runner and
// an in-memory spec stub — no HTTP, no backend dependency. Mirrors the
// Python SDK tests in api/tests/test_elphie_sdk.py.
//
...
// ─── ElphieClient HTTP plumbing (stubbed fetch) ───────────────────────────

describe("ElphieClient", () => {
