...
The generated files live under `elphie_sdk.typed` and are committed to
the repository so `pip install elphie-sdk` ships typed classes without
requiring a regen step.

Run manually:

    python -m elphie_sdk.codegen --api http://localhost:8000 \\
        --out sdk/python/src/elphie_sdk/typed

    python -m elphie_sdk.codegen --input specs.json \\
        --out ./my_typed
"""
...
Regenerate with `python -m elphie_sdk.codegen` against the target
Elphie backend. Source of truth: the backend's model-backed node-spec
...
from elphie_sdk.typed._base import TypedNode
'''
...
def _render_init_module(spec_names: list[str]) -> str:
    lines = [
        ...
        "`from elphie_sdk.typed import StartCall, AgentNode`.",
        '"""',
        "",
    ]
    exports: list[str] = []
    for spec_name in sorted(spec_names):
        ...
        lines.append(f"from elphie_sdk.typed.{module_name} import {class_name}")
        exports.append(class_name)

    lines.append("from elphie_sdk.typed._base import TypedNode")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m elphie_sdk.codegen",
        description="Generate typed SDK dataclasses from the Elphie node-spec catalog.",
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--api", help="Elphie backend base URL")
