...
{{- define "elphie.labels" -}}
helm.sh/chart: {{ include "elphie.chart" . }}
{{ include "elphie.selectorLabels" . }}
{{- if .Chart.AppVersion }}
...
{{- define "elphie.selectorLabels" -}}
app.kubernetes.io/name: {{ include "elphie.name" . }}
app.kubernetes.io/instance: {{ .Release.Name }}
{{- end }}

{{- define "elphie.serviceAccountName" -}}
{{- if .Values.serviceAccount.create }}
{{- default (include "elphie.fullname" .) .Values.serviceAccount.name }}
{{- else }}
...
*/}}
{{- define "elphie.web.fullname" -}}{{ include "elphie.fullname" . }}-web{{- end }}
{{- define "elphie.arqWorker.fullname" -}}{{ include "elphie.fullname" . }}-arq-worker{{- end }}
{{- define "elphie.ariManager.fullname" -}}{{ include "elphie.fullname" . }}-ari-manager{{- end }}
{{- define "elphie.campaignOrchestrator.fullname" -}}{{ include "elphie.fullname" . }}-campaign-orchestrator{{- end }}
{{- define "elphie.ui.fullname" -}}{{ include "elphie.fullname" . }}-ui{{- end }}
{{- define "elphie.coturn.fullname" -}}{{ include "elphie.fullname" . }}-coturn{{- end }}
{{- define "elphie.migrate.fullname" -}}{{ include "elphie.fullname" . }}-migrate{{- end }}

{{- define "elphie.configMapName" -}}{{ include "elphie.fullname" . }}-config{{- end }}
{{- define "elphie.secretName" -}}
{{- if .Values.secrets.existingSecret -}}
{{- .Values.secrets.existingSecret -}}
{{- else -}}
{{- include "elphie.fullname" . }}-secret
{{- end -}}
{{- end }}
...
Called from each template via `include "elphie.deps.resolved" .` (no-op output).
*/}}
{{- define "elphie.deps.resolved" -}}
...
*/}}
{{- define "elphie.postgresHost" -}}{{ .Release.Name }}-postgresql{{- end }}
{{- define "elphie.redisHost" -}}{{ .Release.Name }}-redisinternal-master{{- end }}
{{- define "elphie.minioHost" -}}{{ .Release.Name }}-minio{{- end }}
...
*/}}
{{- define "elphie.postgresPassword" -}}
...
{{- define "elphie.minioRootPassword" -}}
...
password in the <release>-postgresql Secret under key `password`; elphie.dbEnv
projects it into $(POSTGRES_PASSWORD), which this URL interpolates at runtime.
Auth username/database default to `elphie` (see values.postgresql.auth).
*/}}
{{- define "elphie.databaseUrl" -}}
{{- if eq .Values.database.mode "internal" -}}
postgresql+asyncpg://{{ .Values.postgresql.auth.username }}:$(POSTGRES_PASSWORD)@{{ include "elphie.postgresHost" . }}:5432/{{ .Values.postgresql.auth.database }}
{{- else -}}
$(DATABASE_URL)
{{- end -}}
{{- end }}

{{- define "elphie.redisUrl" -}}
{{- if eq .Values.redis.mode "internal" -}}
redis://:$(REDIS_PASSWORD)@{{ include "elphie.redisHost" . }}:6379
{{- else -}}
$(REDIS_URL)
{{- end -}}
{{- end }}
...
embed $(POSTGRES_PASSWORD) / $(REDIS_PASSWORD) (see elphie.databaseUrl /
elphie.redisUrl), so the password vars must come first or the composed URLs
...
{{- end }}
- name: DATABASE_URL
  value: {{ include "elphie.databaseUrl" . | quote }}
- name: REDIS_URL
  value: {{ include "elphie.redisUrl" . | quote }}
...
*/}}
{{- define "elphie.backendEnvFrom" -}}
- configMapRef:
    name: {{ include "elphie.configMapName" . }}
- secretRef:
    name: {{ include "elphie.secretName" . }}
{{- end }}
