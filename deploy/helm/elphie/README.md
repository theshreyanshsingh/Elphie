...
Deploys Elphie on Kubernetes with decomposed backend workloads (web,
...
|------------------------------|-------------|-----------------|-------|
| `elphie-web`                 | 2 (HPA opt) | RollingUpdate   | Long-lived WS, graceful drain |
| `elphie-arq-worker`          | 1 (HPA opt) | RollingUpdate   | Stateless |
| `elphie-ari-manager`         | **1 fixed** | **Recreate**    | Telephony singleton |
| `elphie-campaign-orchestrator` | **1 fixed** | **Recreate**  | Campaign singleton (in-memory locks) |
| `elphie-ui`                  | 2 (HPA opt) | RollingUpdate   | Next.js SSR |
| `elphie-coturn`              | 1           | Recreate        | LoadBalancer Service, port-pinned |
