...
export function ElphieCreditsCard() {
  const auth = useAuth();
  ...
  return (
    <Card className="mb-6">
      <CardHeader>
        <CardTitle>{APP_NAME} Model Credits</CardTitle>
        <CardDescription>
          These track usage of {APP_NAME} models using {APP_NAME} Service Keys.
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoadingCredits ? (
          ...
          <div className="space-y-4">
            ...
            {(mpsCredits.total_quota ?? 0) > 0 && (
              <Progress value={Math.min(100, ((mpsCredits.total_credits_used ?? 0) / (mpsCredits.total_quota ?? 1)) * 100)} className="h-3" />
            )}
          </div>
        ) : (
          <p className="text-muted-foreground">
            No {APP_NAME} service keys configured. Set up a service key in your model configuration to see usage.
          </p>
        )}
