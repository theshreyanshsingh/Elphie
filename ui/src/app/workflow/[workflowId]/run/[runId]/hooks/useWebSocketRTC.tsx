...
// An ICE connection state of "disconnected" is usually a transient network blip
// (Wi-Fi/NAT hiccup) that WebRTC recovers from on its own. Wait this long for it
// to return to "connected" before tearing the call down, so a momentary glitch
// doesn't kill an active conversation.
...
export const useWebSocketRTC = ({ workflowId, workflowRunId, accessToken, initialContextVariables, onNodeTransition }: UseWebSocketRTCProps) => {
    ...
    const clearIceRecoveryTimer = useCallback(() => {
        if (iceRecoveryTimerRef.current !== null) {
    ...
    const getWebSocketUrl = useCallback(async () => {
        ...
        if (configuredBackendUrl) {
            // REST can go through the Next.js /api/v1 proxy on the UI port, but
            // WebSocket upgrades cannot — route signaling directly to the API.
            if (typeof window !== 'undefined' && isLocalhostUi()) {
                try {
    ...
    }, [closePeerConnection, stopLocalStream, clearIceRecoveryTimer]);
    ...
    const createPeerConnection = () => {
        ...
        const handlePeerStateChange = () => {
            ...
            if (pc.connectionState === 'failed' || pc.iceConnectionState === 'failed') {
                // "failed" is terminal — ICE has given up. Tear down immediately.
                clearIceRecoveryTimer();
                cleanupConnection({ graceful: false, status: 'failed' });
                return;
            }

            // "disconnected" is typically a transient blip. Give it a grace period
            // to recover on its own instead of killing the call right away.
            if (
                pc.connectionState === 'disconnected' ||
                pc.iceConnectionState === 'disconnected'
            ) {
                if (iceRecoveryTimerRef.current === null) {
                    logger.warn(`ICE disconnected — waiting up to ${ICE_DISCONNECT_GRACE_MS}ms for recovery before tearing down`);
                    iceRecoveryTimerRef.current = setTimeout(() => {
                        iceRecoveryTimerRef.current = null;
                        const current = pcRef.current;
                        const iceState = current?.iceConnectionState;
                        const connState = current?.connectionState;
                        if (iceState === 'connected' || iceState === 'completed' || connState === 'connected') {
                            logger.info('ICE recovered after transient disconnect');
                            setConnectionStatus('connected');
                            return;
                        }
                        logger.info('ICE did not recover within grace period - cleaning up connection');
            ...
            if (
                ...
                // Fully closed — nothing to recover.
                clearIceRecoveryTimer();
    ...
    }, [stopLocalStream, clearIceRecoveryTimer]);
