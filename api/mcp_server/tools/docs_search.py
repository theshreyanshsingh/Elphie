...
def _resolve_docs_root() -> Path | None:
    ...
    override = os.environ.get("ELPHIE_DOCS_PATH")
    if override:
...
async def list_docs(path: str | None = None, depth: int = 1) -> list[dict]:
    """Browse the Elphie docs hierarchy before reading a page in full.
...
async def search_docs(query: str, limit: int = 5) -> list[dict]:
    """Search the Elphie documentation and return a lean ranked shortlist.
