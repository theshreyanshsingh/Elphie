...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated Elphie user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `elphie.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `elphie.org_id` — that attribute triggers the
