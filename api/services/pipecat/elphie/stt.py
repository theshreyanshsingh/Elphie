...
"""Elphie STT Service implementation using WebSocket streaming."""
...
@dataclass
class ElphieSTTSettings(STTSettings):
    """Settings for ElphieSTTService."""

    pass


class ElphieSTTService(STTService, WebsocketService):
    """Elphie speech-to-text service using WebSocket streaming.

    This service provides real-time speech recognition using Elphie's unified WebSocket API.
    ...
    def __init__(
        ...
        *,
        ...
        settings: ElphieSTTSettings | None = None,
        ttfs_p99_latency: float | None = 0.35,
        **kwargs,
    ):
        ...
        """
        default_settings = ElphieSTTSettings(model="default", language="multi")
    ...
    async def _send_finalize(self):
        """Send finalize message to Elphie server to flush the current transcript."""
        if self._websocket and self._websocket.state == State.OPEN:
    ...
    async def _handle_transcription(self, msg: dict):
        """Process transcription message from Elphie."""
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        """Process frames with Elphie-specific handling.
        ...
        if isinstance(frame, VADUserStoppedSpeakingFrame):
            # Send finalize to flush the current transcript from Deepgram (via Elphie server)
            if self._websocket and self._websocket.state == State.OPEN:
    ...
    async def run_stt(self, audio: bytes) -> AsyncGenerator[Frame | None, None]:
        """Send audio data to Elphie for transcription.
