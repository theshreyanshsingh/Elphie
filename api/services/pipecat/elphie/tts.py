...
"""Elphie TTS Service implementation using WebSocket streaming."""
...
from api.services.pipecat.elphie.mps_billing import (
...
try:
    ...
    logger.error("In order to use Elphie TTS, you need to `pip install websockets`.")
...
def calculate_word_times(
    ...
    Args:
        alignment_info: Word alignment data from Elphie API.
...
@dataclass
class ElphieTTSSettings(TTSSettings):
    """Settings for ElphieTTSService.
...
class ElphieTTSService(WebsocketTTSService):
    """Elphie WebSocket-based TTS service with word timestamps.

    This service provides real-time text-to-speech using Elphie's unified WebSocket API.
    The actual TTS provider (ElevenLabs, OpenAI, Deepgram, etc.) is determined by the
    Elphie backend configuration. Supports word-level timestamps and audio streaming.
    """
    ...
    def __init__(
        ...
        *,
        api_key: str,
        base_url: str = MPS_API_URL.replace("http://", "ws://").replace("https://", "wss://"),
        ws_path: str = "/api/v1/tts/stream",
        correlation_id: str | None = None,
        sample_rate: int | None = None,
        settings: ElphieTTSSettings | None = None,
        text_aggregation_mode: TextAggregationMode | None = None,
        **kwargs,
    ):
        """Initialize Elphie TTS service.

        Args:
            api_key: The Elphie API key for authentication.
            base_url: WebSocket base URL for the Elphie managed service.
        ...
        """
        default_settings = ElphieTTSSettings(
            model="default",
            voice="default",
            language="en",
            speed=1.0,
    ...
    def can_generate_metrics(self) -> bool:
        ...
        Returns:
            True, as Elphie service supports metrics generation.
        """
        return True
    ...
    async def _connect_websocket(self):
        """Establish the websocket connection to Elphie TTS service."""
        try:
            ...
            logger.debug(f"Connecting to Elphie TTS WebSocket at {url}")
            ...
            logger.info(f"Connected to Elphie TTS service")

        except Exception as e:
            self._websocket = None
            logger.error(f"Failed to connect to Elphie TTS service: {e}")
            raise

    async def _disconnect_websocket(self):
        """Close the websocket connection to Elphie TTS service."""
        try:
            await self.stop_all_metrics()

            if self._websocket:
                logger.debug("Disconnecting from Elphie TTS service")
                await self._websocket.close()
                logger.debug("Disconnected from Elphie TTS service")
        except Exception as e:
            logger.error(f"Error disconnecting from Elphie TTS service: {e}")
        finally:
            self._remote_initialized_context_ids.clear()
    ...
    async def _connect(self):
        """Connect to the Elphie TTS service with full initialization."""
    ...
    async def _disconnect(self):
        """Disconnect from Elphie TTS service and clean up tasks."""
    ...
    async def _receive_messages(self):
        """Handle incoming WebSocket messages from Elphie."""
        async for message in self._get_websocket():
            try:
                ...
                # is a response to close_context), Elphie's "final" is sent
                ...
                if msg_type == "audio":
                    ...
                    if is_quota_error:
                        ...
                        raise Exception(f"Elphie TTS error: {error_msg}")

            except asyncio.CancelledError:
                raise
            except json.JSONDecodeError as e:
                logger.error(f"Failed to decode message from Elphie: {e}")
                raise
            except Exception as e:
                logger.error(f"Error processing Elphie TTS message: {e}")
                raise
    ...
    async def run_tts(self, text: str, context_id: str) -> AsyncGenerator[Frame | None, None]:
        """Generate speech from text using Elphie's streaming WebSocket API.
    ...
    async def _close_context(self, context_id: str):
        """Close a Elphie TTS context via WebSocket.
    ...
    async def on_audio_context_interrupted(self, context_id: str):
        """Close the Elphie context when the bot is interrupted."""
        await self._close_context(context_id)

    async def on_audio_context_completed(self, context_id: str):
        """Close the Elphie context after all audio has been played."""
        await self._close_context(context_id)
