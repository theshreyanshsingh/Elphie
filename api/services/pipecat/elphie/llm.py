...
"""Elphie LLM Service implementation using OpenAI-compatible interface."""
...
class ElphieLLMService(OpenAILLMService):
    """A unified LLM service using Elphie's API with OpenAI-compatible interface.

    This service extends OpenAILLMService to connect to Elphie's unified API endpoint
    while maintaining full compatibility with OpenAI's interface. The actual LLM provider
    (OpenAI, Groq, Google, etc.) is determined by the Elphie backend configuration.
    """

    # The Elphie unified endpoint routes to multiple providers, not all of which
    ...
    def __init__(
        ...
        *,
        api_key: str,
        base_url: str = f"{MPS_API_URL.rstrip('/')}/api/v1/llm",
        correlation_id: str | None = None,
        settings: OpenAILLMSettings | None = None,
        **kwargs,
    ):
        """Initialize Elphie LLM service.

        Args:
            api_key: The Elphie API key for authentication.
            base_url: The Elphie managed-service LLM endpoint.
    ...
    def create_client(self, api_key=None, base_url=None, **kwargs):
        """Create OpenAI-compatible client for Elphie API endpoint.
        ...
        Returns:
            An OpenAI-compatible client configured for Elphie's API.
        """
        logger.debug(f"Creating Elphie LLM client with base URL: {base_url or self._base_url}")
    ...
    async def get_chat_completions(
        ...
        """Override to handle Elphie-specific quota errors.
        ...
        try:
            ...
            if "quota_exceeded" in error_str and "403" in error_str:
                ...
                error_msg = "Elphie Service quota exceeded"
