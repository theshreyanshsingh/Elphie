...
Elphie does not rate or deduct credits locally. MPS owns credit accounting.
For hosted deployments, Elphie reports completed platform usage to MPS.
When a server-minted MPS correlation id exists, MPS uses model-service usage
as the canonical duration. Otherwise Elphie reports the completed run duration.
"""
    ...
    try:
        result = await mps_service_key_client.report_platform_usage(
            ...
            metadata={
                ...
                "duration_source": (
                    "mps_correlation" if correlation_id else "elphie_usage_info"
                ),
            },
        )
        logger.info(
