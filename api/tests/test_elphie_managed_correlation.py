...
from api.services.pipecat.elphie.llm import ElphieLLMService
from api.services.pipecat.elphie.stt import ElphieSTTService
from api.services.pipecat.elphie.tts import ElphieTTSService
...
def test_elphie_llm_uses_explicit_mps_correlation_id():
    service = ElphieLLMService(
        api_key="mps-secret",
...
@pytest.mark.asyncio
async def test_elphie_stt_config_uses_explicit_mps_correlation_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "api.services.pipecat.elphie.stt.websocket_connect",
        fake_connect,
    )

    service = ElphieSTTService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=16000,
    )
...
@pytest.mark.asyncio
async def test_elphie_tts_messages_use_explicit_mps_correlation_id(monkeypatch):
    fake_ws = _FakeWebSocket()
    ...
    monkeypatch.setattr(
        "api.services.pipecat.elphie.tts.websocket_connect",
        fake_connect,
    )

    service = ElphieTTSService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=24000,
    )
...
@pytest.mark.asyncio
async def test_elphie_tts_final_for_missing_context_is_ignored():
    service = ElphieTTSService(api_key="mps-secret")
