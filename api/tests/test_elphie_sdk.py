"""Tests for the Python runtime SDK (`elphie_sdk`).
...
import pytest
from elphie_sdk import Workflow
from elphie_sdk._generated_models import NodeSpec
from elphie_sdk.errors import ValidationError
...
class _StubClient:
    """Stand-in for ElphieClient backed by the in-process spec registry.
...
def test_webhook_complex_fields_validate(client: _StubClient):
    ...
    wh = wf.add(
        ...
        custom_headers=[{"key": "X-Source", "value": "elphie"}],
