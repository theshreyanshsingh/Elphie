...
def test_pre_rebrand_managed_configuration_is_normalized():
    legacy_key = "dog" + "rah"
    config = OrganizationAIModelConfigurationV2.model_validate(
        {
            "mode": legacy_key,
            legacy_key: {"api_key": "mps-secret"},
        }
    )

    assert config.mode == "elphie"
    assert config.elphie is not None
    assert config.elphie.api_key == "mps-secret"


def test_pre_rebrand_service_provider_is_normalized():
    ...
    assert config.llm.provider == "elphie"
