...
def _make_active_calls_client(
    monkeypatch,
    configured_secret: str | None = "test-elphie-devops-secret",
) -> TestClient:
    monkeypatch.setattr("api.constants.ELPHIE_DEVOPS_SECRET", configured_secret)
    app = FastAPI()
    app.add_api_route(
...
def test_active_calls_route_requires_configured_secret(monkeypatch):
    ...
    response = client.get(
        "/api/v1/health/active-calls",
        headers={"X-Elphie-Devops-Secret": "test-elphie-devops-secret"},
    )
...
def test_active_calls_route_rejects_wrong_secret(monkeypatch):
    ...
    response = client.get(
        "/api/v1/health/active-calls",
        headers={"X-Elphie-Devops-Secret": "wrong"},
    )
